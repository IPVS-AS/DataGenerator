...
if __name__ == '__main__':
    from FileEntity import FileEntity
else:
...
class Hierarchy(FileEntity):
    """
    Models a Product Hierarchy as a tree of Node objects.
    """

    def __init__(self, name: str, root_node: Node, writeable: bool = True, valid: bool = False):
        """
        It initializes the hierarchy with the given :param root_node:, as well as the root level.
        :param name: The hierarchy's unique name.
        :param root_node: a Node-object to be the root of the hierarchy tree
        :param writeable: [OPTIONAL] default: True. Marks the entity as read-only if set to False.
        :param valid: [OPTIONAL] default: False. Marks if the hierarchy is valid.
        """
        super().__init__(name, writeable)
        self.root = root_node
        self.valid = valid

    def validate(self) -> bool:
        """
        Executes the validation of the hierarchy and sets its "valid" property accordingly.
        :returns: true iff the hierarchy is valid.
        """
        self.valid = self.__validate_node(self.root)
        return self.valid

    def is_valid(self) -> bool:
        """
        Returns the value of its "valid" property without executing the validation again.
        :returns: true iff the hierarchy is valid.
        """
        return self.valid
    ...
    def get_number_of_levels(self) -> int:
        """
        :return: the number of levels defined by the number of edges in the longest path from the root node to a leave,
        + 1 for the root level
        """
        return self.root.height + 1

    @staticmethod
    def __validate_node(node: Node) -> bool:
        """
        Validates the node and all its child nodes recursively.

        A node is valid iff
        * the n_classes property is between 0 (exclusive) and the n_classes property value of the parent node (inclusive).
        * AND the n_samples property equals the sum of all its children's n_samples property (if the node has children).
        """
        classes_valid = Hierarchy.is_classes_sum_valid(node)
        samples_valid = (node.n_samples == Hierarchy.calculate_sample_sum_from_children(node)) \
            if node.has_child_nodes() else True

        node_valid = classes_valid and samples_valid

        if not node_valid:  # A first invalidity is found
            return False
        elif len(node.children) == 0:  # The node is valid and has no children
            return True
        else:  # The node is valid but has children -> recursively check all children
            for child in node.children:
                if not Hierarchy.__validate_node(child):  # An invalidity has been found
                    return False
            return True  # No invalidity found among all its children either

    @staticmethod
    def is_classes_sum_valid(node) -> bool:
        """Validate the n_classes property of the given node.

        The value must be between 0 (exclusive) and the n_classes property value of the parent node (inclusive).

        :param node: Node to validate the n_classes property against.
        :returns: Whether the n_classes value is valid or not.
        :rtype: bool
        """
        if node.parent:
            return 0 < node.n_classes <= node.parent.n_classes
        else:
            return 0 < node.n_classes

    @staticmethod
    def calculate_sample_sum_from_children(node: Node) -> int:
        """Calculate the value of the sample sum property from the child nodes.

        :param node: The node for which to calculate the value.
        :return: The calculated sample sum or 0 if no child nodes are given.
        :rtype: int
        """
        ...
        if node.has_child_nodes():
            ...
            for child_node in node.get_child_nodes():
                sample_sum += child_node.n_samples
            return sample_sum
        else:
            return 0
