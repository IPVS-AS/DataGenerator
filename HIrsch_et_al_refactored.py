...
class SPH(ClassificationMethod):
    ...
    def _get_top_k_accuracy_for_sample(self, y_true, y_score, labels, k=1):
        if y_true in labels:
            if len(y_score) > 2:
...
def save_parameters_and_method_to_dict(accuracy_per_e, parameters, method_name):
    for parameter_key, parameter_value in parameters.items():
        accuracy_per_e[parameter_key] = parameter_value
    accuracy_per_e["Method"] = method_name
    return accuracy_per_e
